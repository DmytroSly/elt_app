...
def stream_and_merge_local_csv() -> None:
    """Demonstrates how to scan folder with csv files, load them in chunk and merge on id column with the previous load"""
    pipeline = dlt.pipeline(
        pipeline_name="standard_filesystem_csv",
        destination='snowflake',
        dataset_name="data_from_filesystem",
        dev_mode=False # When True, a new schema is creatd with every run syffixed with a timestamp
    )
    
    # How to customize schema name in Snowflake? - dataset - is schema, and table_name is a table
    # Why keep two schemas <name>_STAGING and <name>? -staging schems is created for internal purposes of dlt

    # load all csvs
    dp_test_files = filesystem() | read_csv() #readers().read_csv()
    # tell dlt to merge on id
    dp_test_files.apply_hints(write_disposition="merge", merge_key="id")
    # NOTE: we load to met_csv table
