...
DATABASE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'elt_app_metadata.db')
...
class MetadataDB():
    ...
    def _get_entity(self, table: str, column_list: list, id: int | None = None, name: str | None = None) -> tuple | None:
    ...
    def _get_entities(self, table: str, column_list: list):
        columns_str = ', '.join(column_list)
        sql = f'SELECT {columns_str} FROM {table}'
        conn = self.conn
        cursor = conn.cursor()
        result = cursor.execute(sql).fetchall()
        return [dict(zip(column_list, row)) for row in result]
    ...
    def get_platforms(self):
        column_list = ['id', 'name', 'driver_name']  
        platforms = self._get_entities('platform', column_list)
        return platforms
    ...
    def get_connections(self):
        column_list = ['id', 'name', 'platform_id', 'connection_details', 'credentials']  
        connections = self._get_entities('connection', column_list)
        return connections
